...
void help(){
    printf("ZTE N880 Taos proximity sensor calibration program\n");
...
int main(int argc, char **argv){
    ...
    while ((argc_c = getopt(argc, argv, "cdhz")) != -1){
        switch(argc_c){
		...
		case 'c':
		case 'z':
			if(argc_c == 'z') //adde by hehong,from lewa,2011-10-20
			{
				FILE *file = fopen("/data/misc/prox_data.txt","r");
				if(file != NULL)
				{
					fclose(file);
					break;
